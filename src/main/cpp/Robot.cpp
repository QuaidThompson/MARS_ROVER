...
float RampVal(float currentVal, float targetVal, float rampIncriment)
{
  ...
  { // if we are already at the intended value
    return targetVal;
  }
}

float map(float input, float inA, float inb, float outA, float outB)
{ // map function for easy conversion of input to output values
  float output = outA + ((outB - outA) / (inb - inA)) * (input - inA);
  return output;
}

void Robot::setDrivetrain(double rotate, double drive, double mixConstant, bool zeroTurn)
{ // todo: handle false zero turn as well as mix Constant input.
  // cws4
...
{ // ERROR: warn user if there is no controler conected for input and disable all motors
...
void Robot::TeleopPeriodic()
{
  ...
  if (!tooHot)
  {
    ...
    else
    {
      ...
      if (infoUpdateButtonState != prevInfoUpdateButtonState)
      { // ensures user cant hold the button down and constantly update the data
